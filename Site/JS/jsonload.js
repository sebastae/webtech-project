...
function JSONLoader(file, template, callback){
    ...
    request.onreadystatechange = function(){
        if(request.readyState === 4){                               // readyState 4 means that the request has received a response
            if(request.status === 200 || request.status === 0){     // HTTP code 200 means that the content was succesfully fetched
...
function parseTemplate(content, template){
    ...
    while((match = ptrn.exec(template)) != null){   // Assigns the match object to the "match" variable and checks if it is null
